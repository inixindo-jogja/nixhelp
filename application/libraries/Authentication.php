...
class Authentication
{
	...
	private function build_custom_menu( $data, $parrent )
	{
		...
		$str = '<ul class="nav nav-list" id="navigation-menu">';
		$str .= '<li class=""><a href="'. site_url() .'"><i class="menu-picture fa fa-desktop"></i><span class="menu-string"> Dashboard </span></a><b class="arrow"></b></li>';
		if( !empty( $data ) ){
	...
	private function build_cild_custom_menu( $data, $parrent )
	{
		...
		if(isset($data[$parrent])){ 
			foreach($data[$parrent] as $value){		
				...
				$menu_text = ($parrent == 0 ) ? '<span class="menu-string">'. $value['MenuName'] .'</span>' : $value['MenuName'];
				if( $child ){
					$icon = ($value['MenuIconClass'] != '') ? $value['MenuIconClass'] : 'fa-caret-right';
					$str .= '<li class="parent">
								<a href="#" class="dropdown-toggle">
									<i class="menu-picture fa '. $value['MenuIconClass'] .'"></i>
									'. $menu_text .'
					...
					$active = '';
					$current_uri = str_replace($this->ci->router->method ,"", uri_string());
					$module_uri = $value['MenuModule'];
					if($current_uri === $module_uri){
						$active = 'active';
					}
					
					if($value['MenuIconClass'] != ''){
						$str .= '<li class="'. $active .'"><a href="'. site_url( $value['MenuModule'] ) .'"><i class="menu-picture fa '. $value['MenuIconClass'] .'"></i>'. $menu_text .'</a><b class="arrow"></b></li>';	
					} else {
						$str .= '<li class="'. $active .'"><a href="'. site_url( $value['MenuModule'] ) .'"><i class="menu-picture fa fa-caret-right"></i>'. $menu_text .'</a><b class="arrow"></b></li>';	
					}
				}
			}
